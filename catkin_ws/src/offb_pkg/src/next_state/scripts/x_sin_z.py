...
def control():
	...
	np.save('nn1_x_sin_z_input_state.npy',nn1_yz_input_state)
	np.save('nn1_x_sin_z_grd_truth.npy',nn1_yz_grd_truth)
	np.save('nn2_x_sin_z_input_state.npy',nn2_yz_input_state)
	np.save('nn2_x_sin_z_grd_truth.npy',nn2_yz_grd_truth)

	s_x_sin_z = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_x_sin_z = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_x_sin_z = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_x_sin_z :',s_x_sin_z.shape)
	print('u_x_sin_z :',u_x_sin_z.shape)
	print('a_x_sin_z :',a_x_sin_z.shape)


	np.save('s_x_sin_z.npy' ,s_x_sin_z)
	np.save('u_x_sin_z.npy' ,u_x_sin_z)
	np.save('a_x_sin_z.npy' ,a_x_sin_z)
