...
def control():
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			print('data_points =', data_points, " count =  ",count, "row =",row)
	...
	nn1_yaw_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u3],ndmin=2).transpose()
	nn1_yaw_grd_truth = np.array([a_x,a_y,a_z],ndmin=2).transpose()

	nn2_yaw_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u0, u1, u2],ndmin=2).transpose()
	nn2_yaw_grd_truth  = np.array([aplha_x,aplha_y,aplha_z],ndmin=2).transpose()


	print('nn1_rand_x_neg_y_input_state   :',nn1_yaw_input_state.shape)
	print('nn1_rand_x_neg_y_grd_truth   :',nn1_yaw_grd_truth.shape)
	print('nn2_rand_x_neg_y_input_state   :',nn2_yaw_input_state.shape)
	print('nn2_rand_x_neg_y_grd_truth :',nn2_yaw_grd_truth.shape)

	np.save('nn1_rand_x_neg_y_input_state.npy',nn1_yaw_input_state)
	np.save('nn1_rand_x_neg_y_grd_truth.npy',nn1_yaw_grd_truth)
	np.save('nn2_rand_x_neg_y_input_state.npy',nn2_yaw_input_state)
	np.save('nn2_rand_x_neg_y_grd_truth.npy',nn2_yaw_grd_truth)

	s_rand_x_neg_y = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_rand_x_neg_y = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_rand_x_neg_y = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_rand_x_neg_y :',s_rand_x_neg_y.shape)
	print('u_rand_x_neg_y :',u_rand_x_neg_y.shape)
	print('a_rand_x_neg_y :',a_rand_x_neg_y.shape)


	np.save('s_rand_x_neg_y.npy' ,s_rand_x_neg_y)
	np.save('u_rand_x_neg_y.npy' ,u_rand_x_neg_y)
	np.save('a_rand_x_neg_y.npy' ,a_rand_x_neg_y)
