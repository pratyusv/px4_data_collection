...
def control():
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			pose.pose.position.x = 3*mt.cos(theta*PI/6)
			pose.pose.position.y = 3*mt.sin(theta*PI/6)
			pose.pose.position.z = 3
	...
	np.save('nn1_circle_input_state.npy',nn1_xy_input_state)
	np.save('nn1_circle_grd_truth.npy',nn1_xy_grd_truth)
	np.save('nn2_circle_input_state.npy',nn2_xy_input_state)
	np.save('nn2_circle_grd_truth.npy',nn2_xy_grd_truth)

	s_circle = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_circle = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_circle = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_circle :',s_circle.shape)
	print('u_circle :',u_circle.shape)
	print('a_circle :',a_circle.shape)


	np.save('s_circle.npy' ,s_circle)
	np.save('u_circle.npy' ,u_circle)
	np.save('a_circle.npy' ,a_circle)
