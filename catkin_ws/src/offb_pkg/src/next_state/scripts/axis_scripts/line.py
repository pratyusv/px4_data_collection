...
def control():
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			print('data_points =', data_points, " count =  ",count)
	...
	print('nn1_line_input_state   :',nn1_yaw_input_state.shape)
	print('nn1_line_grd_truth   :',nn1_yaw_grd_truth.shape)
	print('nn2_line_input_state   :',nn2_yaw_input_state.shape)
	print('nn2_line_grd_truth :',nn2_yaw_grd_truth.shape)

	np.save('nn1_line_input_state.npy',nn1_yaw_input_state)
	np.save('nn1_line_grd_truth.npy',nn1_yaw_grd_truth)
	np.save('nn2_line_input_state.npy',nn2_yaw_input_state)
	np.save('nn2_line_grd_truth.npy',nn2_yaw_grd_truth)

	s_line = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_line = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_line = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_line :',s_line.shape)
	print('u_line :',u_line.shape)
	print('a_line :',a_line.shape)


	np.save('s_line.npy' ,s_line)
	np.save('u_line.npy' ,u_line)
	np.save('a_line.npy' ,a_line)
