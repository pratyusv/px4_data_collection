...
def control():
	...
	nn1_y_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u3],ndmin=2).transpose()
	nn1_y_grd_truth = np.array([a_x,a_y,a_z],ndmin=2).transpose()

	nn2_y_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u0, u1, u2],ndmin=2).transpose()
	nn2_y_grd_truth  = np.array([aplha_x,aplha_y,aplha_z],ndmin=2).transpose()


	print('nn1_y_input_state   :',nn1_y_input_state.shape)
	print('nn1_y_grd_truth   :',nn1_y_grd_truth.shape)
	print('nn2_y_input_state   :',nn2_y_input_state.shape)
	print('nn2_y_grd_truth :',nn2_y_grd_truth.shape)

	np.save('nn1_y_input_state.npy',nn1_y_input_state)
	np.save('nn1_y_grd_truth.npy',nn1_y_grd_truth)
	np.save('nn2_y_input_state.npy',nn2_y_input_state)
	np.save('nn2_y_grd_truth.npy',nn2_y_grd_truth)

	s_y = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_y = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	ac_y = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_y :',s_y.shape)
	print('u_y :',u_y.shape)
	print('ac_y :',ac_y.shape)


	np.save('s_y.npy' ,s_y)
	np.save('u_y.npy' ,u_y)
	np.save('a_y.npy' ,ac_y)
