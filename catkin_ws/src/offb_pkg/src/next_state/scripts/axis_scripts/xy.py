...
def control():
	...
	np.save('nn1_xy_input_state.npy',nn1_xy_input_state)
	np.save('nn1_xy_grd_truth.npy',nn1_xy_grd_truth)
	np.save('nn2_xy_input_state.npy',nn2_xy_input_state)
	np.save('nn2_xy_grd_truth.npy',nn2_xy_grd_truth)

	s_xy = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_xy = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_xy = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_xy :',s_xy.shape)
	print('u_xy :',u_xy.shape)
	print('a_xy :',a_xy.shape)

	np.save('s_xy.npy' ,s_xy)
	np.save('u_xy.npy' ,u_xy)
	np.save('a_xy.npy' ,a_xy)
