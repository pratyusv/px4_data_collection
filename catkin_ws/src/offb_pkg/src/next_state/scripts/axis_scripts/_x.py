...
def control():
	...
	nn1_neg_x_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u3],ndmin=2).transpose()
	nn1_neg_x_grd_truth = np.array([a_x,a_y,a_z],ndmin=2).transpose()

	nn2_neg_x_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u0, u1, u2],ndmin=2).transpose()
	nn2_neg_x_grd_truth  = np.array([aplha_x,aplha_y,aplha_z],ndmin=2).transpose()



	print('nn1_neg_x_input_state   :',nn1_neg_x_input_state.shape)
	print('nn1_neg_x_grd_truth   :',nn1_neg_x_grd_truth.shape)
	print('nn2_neg_x_input_state   :',nn2_neg_x_input_state.shape)
	print('nn2_neg_x_grd_truth :',nn2_neg_x_grd_truth.shape)

	np.save('nn1_neg_x_input_state.npy',nn1_neg_x_input_state)
	np.save('nn1_neg_x_grd_truth.npy',nn1_neg_x_grd_truth)
	np.save('nn2_neg_x_input_state.npy',nn2_neg_x_input_state)
	np.save('nn2_neg_x_grd_truth.npy',nn2_neg_x_grd_truth)


	s_neg_x = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_neg_x = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_neg_x = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()

	print('s_neg_x :',s_neg_x.shape)
	print('u_neg_x :',u_neg_x.shape)

	np.save('s_neg_x.npy' ,s_neg_x)
	np.save('u_neg_x.npy' ,u_neg_x)
	np.save('a_neg_x.npy' ,a_neg_x)
