...
def control():
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			pose.pose.position.x = theta
			pose.pose.position.y = 2.0*mt.cos(theta*PI/6) - 2 
			pose.pose.position.z = 3
	...
	np.save('nn1_x_cos_y_input_state.npy',nn1_xy_input_state)
	np.save('nn1_x_cos_y_grd_truth.npy',nn1_xy_grd_truth)
	np.save('nn2_x_cos_y_input_state.npy',nn2_xy_input_state)
	np.save('nn2_x_cos_y_grd_truth.npy',nn2_xy_grd_truth)

	s_x_cos_y = np.array([x_f, y_f, z_f,vx_f,vy_f,vz_f,sin_r_f,sin_p_f,sin_y_f,cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f,r_f,p_f,yaw_f],ndmin=2).transpose()
	u_x_cos_y = np.array([u0,u1,u2,u3],ndmin=2).transpose()
	a_x_cos_y = np.array([ax_f,ay_f,az_f],ndmin=2).transpose()


	print('s_x_cos_y :',s_x_cos_y.shape)
	print('u_x_cos_y :',u_x_cos_y.shape)
	print('a_x_cos_y :',a_x_cos_y.shape)


	np.save('s_x_cos_y.npy' ,s_x_cos_y)
	np.save('u_x_cos_y.npy' ,u_x_cos_y)
	np.save('a_x_cos_y.npy' ,a_x_cos_y)
