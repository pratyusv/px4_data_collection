...
def control():
	...
	nn1_xz_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u3],ndmin=2).transpose()
	nn1_xz_grd_truth = np.array([a_x,a_y,a_z],ndmin=2).transpose()

	nn2_xz_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f, u0, u1, u2],ndmin=2).transpose()
	nn2_xz_grd_truth  = np.array([aplha_x,aplha_y,aplha_z],ndmin=2).transpose()
	...
	np.save('nn1_60_sin_xz_input_state.npy',nn1_xz_input_state)
	np.save('nn1_60_sin_xz_grd_truth.npy',nn1_xz_grd_truth)
	np.save('nn2_60_sin_xz_input_state.npy',nn2_xz_input_state)
	np.save('nn2_60_sin_xz_grd_truth.npy',nn2_xz_grd_truth)

	s_xz_sin = np.array([x_f, y_f, z_f, vx_f,vy_f,vz_f,sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f],ndmin=2).transpose()
	u_xz_sin = np.array([u0,u1,u2,u3],ndmin=2).transpose()

	print('s_xz_sin :',s_xz_sin.shape)
	print('u_xz_sin :',u_xz_sin.shape)

	np.save('s_60_xz_sin.npy' ,s_xz_sin)
	np.save('u_60_xz_sin.npy' ,u_xz_sin)
