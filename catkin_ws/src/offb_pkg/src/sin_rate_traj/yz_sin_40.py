...
def control():
	...
	local_pos_sub = rospy.Subscriber("/mavros/local_position/pose", PoseStamped, lp_cb, queue_size=1)
	local_vel_sub = rospy.Subscriber("/mavros/local_position/velocity", TwistStamped, lv_cb, queue_size=1)
	act_control_sub = rospy.Subscriber("/mavros/act_control/act_control_pub", ActuatorControl,act_cb,queue_size=1)
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			cos_yz_sin_f.append(mt.cos(float(mt.radians(yaw))))
	...
	nn1_yz_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_yz_sin_f, u3],ndmin=2).transpose()
	nn1_yz_grd_truth = np.array([a_x,a_y,a_z],ndmin=2).transpose()

	nn2_yz_input_state = np.array([vx_f,vy_f,vz_f, rs_f,ps_f,ys_f, sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_yz_sin_f, u0, u1, u2],ndmin=2).transpose()
	...
	np.save('nn1_40_sin_yz_input_state.npy',nn1_yz_input_state)
	np.save('nn1_40_sin_yz_grd_truth.npy',nn1_yz_grd_truth)
	np.save('nn2_40_sin_yz_input_state.npy',nn2_yz_input_state)
	np.save('nn2_40_sin_yz_grd_truth.npy',nn2_yz_grd_truth)

	s_yz_sin = np.array([x_f, y_f, z_f, vx_f,vy_f,vz_f,sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_yz_sin_f,rs_f,ps_f,ys_f],ndmin=2).transpose()
	u_yz_sin = np.array([u0,u1,u2,u3],ndmin=2).transpose()

	print('s_yz_sin :',s_yz_sin.shape)
	print('u_yz_sin :',u_yz_sin.shape)

	np.save('s_40_yz_sin.npy' ,s_yz_sin)
	np.save('u_40_yz_sin.npy' ,u_yz_sin)
