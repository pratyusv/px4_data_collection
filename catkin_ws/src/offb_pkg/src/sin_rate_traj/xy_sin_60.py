...
def control():
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			x_des.append(theta)
			y_des.append(6.0*mt.sin(theta*PI/6))
			z_des.append(3)
	...
	np.save('nn1_60_sin_xy_input_state.npy',nn1_xy_input_state)
	np.save('nn1_60_sin_xy_grd_truth.npy',nn1_xy_grd_truth)
	np.save('nn2_60_sin_xy_input_state.npy',nn2_xy_input_state)
	np.save('nn2_60_sin_xy_grd_truth.npy',nn2_xy_grd_truth)

	s_xy_sin = np.array([x_f, y_f, z_f, vx_f,vy_f,vz_f,sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f,rs_f,ps_f,ys_f],ndmin=2).transpose()
	u_xy_sin = np.array([u0,u1,u2,u3],ndmin=2).transpose()

	print('s_xy_sin :',s_xy_sin.shape)
	print('u_xy_sin :',u_xy_sin.shape)

	np.save('s_60_xy_sin.npy' ,s_xy_sin)
	np.save('u_60_xy_sin.npy' ,u_xy_sin)
