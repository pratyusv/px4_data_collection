...
def control():
	...
	imu_sub = rospy.Subscriber("/mavros/imu/data",Imu,imu_cb, queue_size=100000)
	local_pos_sub = rospy.Subscriber("/mavros/local_position/pose", PoseStamped, lp_cb, queue_size=None)
	local_vel_sub = rospy.Subscriber("/mavros/local_position/velocity", TwistStamped, lv_cb, queue_size=None)
	act_control_sub = rospy.Subscriber("/mavros/act_control/act_control_pub", ActuatorControl,act_cb,queue_size=None)
	...
	while not rospy.is_shutdown():
		...
		if flag1 and flag2:
			...
			# pose.pose.position.x = 6.0*mt.sin(theta*PI/6)
			# pose.pose.position.y = theta 
			# pose.pose.position.z = 3

			pose.pose.position.x = 0
			pose.pose.position.y = theta 
			pose.pose.position.z = 6.0 + 2.0*mt.sin(theta*PI/6)
	...
	np.save('nn1_sin_xy_input_state.npy',nn1_xy_input_state)
	np.save('nn1_sin_xy_grd_truth.npy',nn1_xy_grd_truth)
	np.save('nn2_sin_xy_input_state.npy',nn2_xy_input_state)
	np.save('nn2_sin_xy_grd_truth.npy',nn2_xy_grd_truth)
	...
	# vx_f = vx_f[1:len(vx_f)]
	# vy_f = vy_f[1:len(vy_f)]
	# vz_f = vz_f[1:len(vz_f)]

	
	s_xy_sin = np.array([x_f, y_f, z_f, vx_f,vy_f,vz_f,sin_r_f, sin_p_f,sin_y_f, cos_r_f,cos_p_f,cos_y_f,r_f, p_f,yaw_f ,rs_f,ps_f,ys_f],ndmin=2).transpose()
	# s_xy_sin = np.array([x_f, y_f, z_f, vx_f,vy_f,vz_f,sin_r_f[0:len(sin_r_f)-1], sin_p_f[0:len(sin_p_f)-1],sin_y_f[0:len(sin_y_f)-1], cos_r_f[0:len(cos_r_f)-1],cos_p_f[0:len(cos_p_f)-1],cos_y_f[0:len(cos_y_f)-1],r_f[0:len(r_f)-1], p_f[0:len(p_f)-1],yaw_f[0:len(yaw_f)-1],rs_f[0:len(rs_f)-1],ps_f[0:len(ps_f)-1],ys_f[0:len(ys_f)-1]],ndmin=2).transpose()
	# a_xy_sin = np.array([ ax_f[0:len(ax_f)],ay_f[0:len(ay_f)],az_f[0:len(az_f)]],ndmin=2).transpose()
	a_xy_sin = np.array([ ax_f,ay_f,az_f],ndmin=2).transpose()
	u_xy_sin = np.array([u0,u1,u2,u3],ndmin=2).transpose()

	print('s_xy_sin :',s_xy_sin.shape)
	print('u_xy_sin :',u_xy_sin.shape)
	print('a_xy_sin :',a_xy_sin.shape)

	np.save('s_xy_sin.npy', s_xy_sin)
	np.save('u_xy_sin.npy', u_xy_sin)
	np.save('a_xy_sin.npy', a_xy_sin)
